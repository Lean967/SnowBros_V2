...
public class Movediza extends Plataforma {
    ...
    public void hacerEfecto(SnowBro snowbro) { /*ESTO HAY QUE REVISAR YA QUE NO LOGRA COLISIONAR, UNA VEZ LOGRADO BORRAR ESTE COMENTARIO */
        ...
        snowbro.getPosicion().setY(posicion.getY() - (int)snowbro.getBounds().getHeight()+2);
    }

    public void hacerEfecto(Enemigo enemigo) {
        int enemigoBaseY = enemigo.getPosicion().getY() + enemigo.getBounds().height;
        if(enemigo.getPasoY() >= 0 && enemigoBaseY - enemigo.getPasoY() <= getPosicion().getY()+1){
            enemigo.setCayendo(false);
            enemigo.getPosicion().setY(posicion.getY() - (int)enemigo.getBounds().getHeight() + 2);
        }
    ...
    protected Movediza crearPlataforma(){
        ...
        if(this.pasoY != 0){
            nuevaMovediza.configurarComoAscensor(limiteSuperior, limiteInferior, velocidadMovimiento);
        }else{
            nuevaMovediza.configurarComoLateral(limiteIzquierdo, limiteDerecho, velocidadMovimiento);
        }
